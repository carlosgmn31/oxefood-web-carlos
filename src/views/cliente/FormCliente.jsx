...
export default function FormCliente() {
  ...
  useEffect(() => {
    if (state != null && state.id != null) {
      axios.get('http://localhost:8080/api/cliente/' + state.id).then((response) => {
  ...
  function salvar() {
    ...
    if (idCliente != null) {
      ...
      axios
        ...
        }, setTimeout(navigate('/list-cliente?nocache=' + new Date().getTime())))
        .catch((error) => {
          ...
          <div style={{ marginTop: '4%' }}>
            <Form>
              <Form.Group widths="equal">
                <Form.Input required fluid label="Nome" maxLength="100" value={nome} onChange={(e) => setNome(e.target.value)} />
              ...
              <Form.Group>
                <Form.Input fluid label="Fone Celular" width={6}>
                  <InputMask mask="(99) 9999.9999" onChange={(e) => setFoneCelular(e.target.value)} value={foneCelular} />
                </Form.Input>

                <Form.Input fluid label="Fone Fixo" width={6}>
                  <InputMask onChange={(e) => setFoneFixo(e.target.value)} value={foneFixo} mask="(99) 9999.9999" />
                </Form.Input>

                <Form.Input fluid label="Data Nascimento" width={6}>
                  <InputMask mask="99/99/9999" maskChar={null} value={dataNascimento} onChange={(e) => setDataNascimento(e.target.value)} placeholder="Ex: 20/03/1985" />
                </Form.Input>
              </Form.Group>
            </Form>
