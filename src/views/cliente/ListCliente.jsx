...
export default function ListCliente() {
  ...
  function salvarEndereco() {
    ...
    if (enderecoSelecionado != null && clienteSelecionado != null) {
      //Alteração:
      axios
        .put('http://localhost:8080/api/cliente/endereco/' + enderecoSelecionado.id, enderecoRequest)
        .then((response) => {
          console.log('Endereço alterado com sucesso.');
        }, recarregarEnderecos())
        .catch((error) => {
          console.log('Erro ao alterar um Endereço.');
        });
    }
    if (clienteSelecionado != null) {
      ...
      axios
        .post('http://localhost:8080/api/cliente/endereco/' + clienteSelecionado.id, enderecoRequest)
        .then(
          (response) => {
            console.log('Endereço cadastrado com sucesso.');
          },
          axios.get('http://localhost:8080/api/cliente/' + clienteSelecionado.id).then((response) => {
  ...
  return (
    <div>
                  ...
                  <Table.Row key={cliente.id}>
                    ...
                    <Table.Cell textAlign="center">
                      ...
                      <Button inverted circular color="green" title="Clique aqui para editar os dados deste cliente" icon>
                        <Link to="/form-cliente" state={{ id: cliente.id, enderecos: cliente.enderecos[0] }} style={{ color: 'green' }}>
                          {' '}
                          <Icon name="edit" />{' '}
                        </Link>
                      </Button>{' '}
                      &nbsp;
                      <Button inverted circular color="red" title="Clique aqui para remover este cliente" onClick={(e) => confirmaRemover(cliente.id, 'cliente')} icon>
                        <Icon name="trash" />
                      </Button>
                    </Table.Cell>
                  </Table.Row>
      ...
      <div>
                ...
                <Table.Body>
                  {clienteSelecionado.enderecos.map((endereco) => (
                    <Table.Row key={endereco.id}>
                      ...
                      <Table.Cell textAlign="center">
                        <Button inverted circular color="green" title="Clique aqui para editar os dados deste endereço" onClick={() => setandoRegistro('edit', endereco)} icon>
                          <Icon name="edit" />
                        </Button>{' '}
                        &nbsp;
                        <Button inverted circular color="red" title="Clique aqui para remover este endereço" icon onClick={() => confirmaRemover(endereco.id, 'endereco')}>
                          <Icon name="trash" />
                        </Button>
                        &nbsp;
                        {/* <Button
        ...
        <Modal basic onClose={() => setOpenModalCadastroEndereco(false)} onOpen={() => setOpenModalCadastroEndereco(true)} open={openModalCadastroEndereco}>
          ...
          <Form inverted>
            <Form.Group>
              <Form.Input fluid label="Rua" width={6} value={rua} onChange={(e) => setRua(e.target.value)} />
              <Form.Input fluid label="Número" width={6} value={numero} onChange={(e) => setNumero(e.target.value)} />
              <Form.Input fluid label="Bairro" width={6} value={bairro} onChange={(e) => setBairro(e.target.value)} />
            </Form.Group>
            <Form.Group>
              <Form.Input fluid label="CEP" width={6} value={cep} onChange={(e) => setCep(e.target.value)} />
              <Form.Input fluid label="Cidade" width={6} value={cidade} onChange={(e) => setCidade(e.target.value)} />
              <Form.Input fluid label="Estado" width={6} value={estado} onChange={(e) => setEstado(e.target.value)} />
            </Form.Group>
            <Form.Group>
              <Form.Input fluid label="Complemento" width={6} value={complemento} onChange={(e) => setComplemento(e.target.value)} />
            </Form.Group>
          </Form>
          <Modal.Actions>
            ...
            <Button color="red" onClick={() => setOpenModalCadastroEndereco(false)}>
              <Icon name="remove" /> Fechar
            </Button>
          </Modal.Actions>
        </Modal>
