...
export default function FormCategoriaProduto() {
  ...
  useEffect(() => {
    if (state != null && state.id != null) {
      axios.get('http://localhost:8080/api/categoriaproduto/' + state.id).then((response) => {
  ...
  function salvar() {
    ...
    if (idCategoriaProduto != null) {
      ...
      axios
        ...
        .catch((error) => {
          notifyError('Erro ao alterar uma categoria de produto.', JSON.stringify(error, null, 2));
        });
    } else {
      //Cadastro:
      axios
        ...
        .catch((error) => {
          notifyError('Erro ao incluir a categoria de produto.', JSON.stringify(error, null, 2));
        });
    }
  }
  return (
    <div>
      ...
      <Container textAlign="justified">
        ...
        <Button color="orange" onClick={salvar}>
          Salvar
        </Button>
      </Container>
    </div>
